...
const MemberDash = () => {
  ...
  const fetchChannels = async () => {
    ...
    const channelList = querySnapshot.docs.map((doc) => ({ id: doc.id, ...doc.data() }));
  ...
  const sendMessage = async () => {
    ...
    const messagesRef = collection(db, "chats", chatId, "messages");
    await addDoc(messagesRef, { text: newMessage, sender: user.email, timestamp: serverTimestamp() });
    setNewMessage("");
  };
      ...
      <div>
        ...
        <ul>
          {searchResults.map((result) => (
            <li key={result.id}>
              {result.email} <button onClick={() => addFriend(result)}>Add</button>
            </li>
          ))}
        </ul>
      </div>
        ...
        <ul>
          {friends.length > 0 ? (
            friends.map((friend, index) => (
              <li key={index} onClick={() => selectFriend(friend)}>{friend}</li>
            ))
          ) : (
